        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav me-auto mb-lg-0">
                ...
                if (!in_array("dashboard_customization", $hidden_topbar_menus) && (get_setting("disable_new_dashboard_icon") != 1)) {
                    if ($login_user->is_admin) { //valido que solo los admin  puedan cambiar configiracion del panel
            ...
            <div class="d-flex w-auto">
                <ul class="navbar-nav">
                    ...
                    <?php
                    if (!in_array("quick_add", $hidden_topbar_menus) && $login_user->is_admin) {
                    ...
                    <?php if (get_setting("module_message") && can_access_messages_module()) { ?>
                        <li
                            class="nav-item dropdown hidden-sm <?php echo ($login_user->user_type === "client" && !get_setting("client_message_users")) ? "hide" : ""; ?>">
                    ...
                    <li class="nav-item dropdown">
                        <a id="user-dropdown" href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown"
                            ...
                            <span
                                class="user-name ml10"><?php echo $login_user->first_name . " " . $login_user->last_name; ?></span>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end w200 user-dropdown-menu">
                            <?php if ($login_user->user_type == "client") { ?>
                                <div class="company-switch-option d-none">
                                    <?php show_clients_of_this_client_contact($login_user, true); ?></div>
                                <li><?php echo get_client_contact_profile_link($login_user->id . '/general', "<i data-feather='user' class='icon-16 me-2'></i>" . app_lang('my_profile'), array("class" => "dropdown-item")); ?>
                                </li>
                                <li><?php echo get_client_contact_profile_link($login_user->id . '/account', "<i data-feather='key' class='icon-16 me-2'></i>" . app_lang('change_password'), array("class" => "dropdown-item")); ?>
                                </li>
                                <li><?php echo get_client_contact_profile_link($login_user->id . '/my_preferences', "<i data-feather='settings' class='icon-16 me-2'></i>" . app_lang('my_preferences'), array("class" => "dropdown-item")); ?>
                                </li>
                            <?php } else { ?>
                                <li><?php echo get_team_member_profile_link($login_user->id . '/general', "<i data-feather='user' class='icon-16 me-2'></i>" . app_lang('my_profile'), array("class" => "dropdown-item")); ?>
                                </li>
                                <li><?php echo get_team_member_profile_link($login_user->id . '/account', "<i data-feather='key' class='icon-16 me-2'></i>" . app_lang('change_password'), array("class" => "dropdown-item")); ?>
                                </li>
                                <li><?php echo get_team_member_profile_link($login_user->id . '/my_preferences', "<i data-feather='settings' class='icon-16 me-2'></i>" . app_lang('my_preferences'), array("class" => "dropdown-item")); ?>
                                </li>
                            <?php } ?>
                            ...
                            <li class="dropdown-divider"></li>
                            <li><a href="<?php echo_uri('signin/sign_out'); ?>" class="dropdown-item"><i
                                        data-feather="log-out" class='icon-16 me-2'></i>
                                    <?php echo app_lang('sign_out'); ?></a></li>
                        </ul>
                    </li>
                </ul>
            </div>
...
<!-- processingModal: está disponible en TODO el sitio -->
<style>
    .modal-backdrop.backdrop-front {
    z-index: 19999 !important;
    }
    #processingModal.modal-front {
    z-index: 20000 !important;
    }
</style>
<div class="modal fade modal-front" id="processingModal" tabindex="-1" >
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content text-center p-3">
            <div>
                <div id="processingModalBody">
                    <!-- Spinner -->
                    <div id="processingSpinner">
                        <div class="spinner-border" role="status"></div>
                    </div>
                    <div class="errorProcess">
                    ...
                    <!-- Mensaje -->
                    <p id="processingModalMessage" class="mt-2 mb-0">Procesando...</p>
                </div>
            </div>
            <div class="modal-footer justify-content-center border-0 pt-0">
                <button type="button" id="processingModalButton" class="btn btn-primary" data-bs-dismiss="modal">
                    Aceptar
                </button>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        messageOptions.notificationStatusUpdateUrl =
            "<?php echo_uri('messages/update_notification_checking_status'); ?>";
        ...
        notificationOptions.notificationStatusUpdateUrl =
            "<?php echo_uri('notifications/update_notification_checking_status'); ?>";
        notificationOptions.checkNotificationAfterEvery =
            "<?php echo get_setting('check_notification_after_every'); ?>";
        ...
        notificationOptions.pushNotification =
            "<?php echo get_setting("enable_push_notification") && $login_user->enable_web_notification && !get_setting('user_' . $login_user->id . '_disable_push_notification') ? true : false ?>";
        ...
        $("body").on("click",
            ".notification-dropdown a[data-act='ajax-modal'], #js-quick-add-task, #js-quick-add-multiple-task, #task-details-edit-btn, #task-modal-view-link",
            function() {
