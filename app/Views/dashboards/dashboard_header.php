...
<?php if($login_user->is_admin && isset($role_options)) { ?>

<?php echo form_open(get_uri("dashboard/save_custom_widget_role"), array("id" => "custom_widget-form", "class" => "general-form", "role" => "form")); ?>

<div class="card">
    <div class="card-header">
        <label for=""><b>Asigna ésta plantilla a un rol en especifico. La usará predeterminadamente.</b></label>
    </div>
    <div class="card-body">
        <input type="hidden" name="panel_id" value="<?php echo $dashboard_info->id ?>" />
        <div class="form-group">
            <div class="row">
                <div class=" col-md-9">
                    <select name="role_id" id="role_select" class="form-control" required>
                        <option value="0">Selecciona el rol que utilizará esta plantilla.</option>
                        <?php foreach ($role_options as $role): ?>
                        <option value="<?php echo $role->id; ?>"
                            <?php echo ($role_select != "" && $role_select == $role->id) ? "selected" : ""; ?>>
                            <?php echo $role->title; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
