        ...
        <div class="col-md-2 col-sm-6">
            <div class="card">
                <div class="card-body cart-grid-item p0">
                    ...
                    <div class="p15">
                        <div class="font-16 text-wrap-ellipsis strong" style="color: #0044cc;"><?php echo $item->title; ?></div>
                        <div class="mt5 cart-item-rate">
                            <span style="font-size: 25px;"><?php echo to_currency($item->rate); ?></span><span class="text-off font-11"><?php echo $item->unit_type ? "/" . $item->unit_type : ""; ?></span>
                        </div>
                        <div class="text-wrap-ellipsis mt5"><?php echo $item->description ? process_images_from_content($item->description) : "-"; ?></div>
                    </div>
                </div>
                <div class="card-footer no-border text-center p0">
                    <?php
                    if (isset($item->added_to_cart) && $item->added_to_cart) {
                        ...
                        echo js_anchor(app_lang("add_to_cart"), array("class" => "btn btn-xs btn-white text-white item-add-to-cart-btn p15 w100p", "data-item_id" => $item->id));
                    }
                    ?>
                </div>
            </div>
