...
if ($notification->daily_report_id && $notification->clinic_title) {
    echo "<div class='notify-items-url' id='" . $notification->daily_report_id . "'> " . $notification->clinic_title . " <b>" . app_lang("see_here") . "</b></div>";

    echo '<script>
const notifyItems = document.querySelectorAll(".notify-items-url");
...
notifyItems.forEach(item => {
    ...
    if (id) {
        ...
        if (parent) {
            parent.setAttribute("href", "' . get_uri('daily_report?report=') . '" + id);
        }
    }
});
</script>';
...
if ($notification->stamp_id) {
    echo '<div
        class="notify-items-url"
        data-uri="' . get_uri('stamp') . '">
        ' . $notification->clinic_select . ' <b> #'.$notification->stamp_id. " ". app_lang('see_here') . '</b>
    </div>';

    echo '<script>
    document.querySelectorAll(".notify-items-url").forEach(item => {
        const uri = item.dataset.uri;
        const parent = item.closest(".list-group-item");
        if (parent && uri) {
            parent.setAttribute("href", uri);
        }
    });
    </script>';
}
...
if ($notification->excuse_id) {
    echo "<div class='notify-excuse-url' id='" . $notification->excuse_id . "'></div>";

    echo '<script>
    const notifyItemsExcuse = document.querySelectorAll(".notify-excuse-url");

    notifyItemsExcuse.forEach(item => {
        const id = item.getAttribute("id");
        ...
        if (id) {
            ...
            if (parent) {
                parent.setAttribute("href", "' . get_uri('excuse') . '");
            }
        }
    });
    </script>';
...
try {
    ...
    $notification_descriptions = app_hooks()->apply_filters(
        'app_filter_notification_description',
