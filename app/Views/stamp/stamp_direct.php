    ...
    <div class="card" style="min-height:90vh;">
        <div class="card-title d-flex align-items-center">
            ...
            <h3 class="flex-grow-1 text-center mb-0">
                <span class="badge badge-primary">Timbre Directo</span>
            </h3>
        </div>
        <hr>
        ...
        <div class="row">
                ...
                <div id="wizard" class="m-4">
                    ...
                    <div class="wizard-header">
                        <h4>
                            ...
                            <span id="step-title">Subir documento escaneado</span>
                        </h4>
                    </div>
                        ...
                        <form id="templateWizardForm" method="post" enctype="multipart/form-data">
                            ...
                            <div class="wizard-step" data-step="5">
                                <div class="text-center mb-3">
                                    <img src="<?= base_url('assets/images/escaneada.png') ?>" alt="Documento escaneado" class="rounded shadow" style="max-width: 250px;">
                                    <h4 class="mt-3 text-success fw-bold">
                                        ¡Subistes un Documento escaneado exitosamente!
                                    </h4>
                                    <p class="text-muted mb-0">Continuemos para ubicar la firma digital y finalizar el timbre.</p>
                                </div>

                                <div class="form-group mt-4 d-none">
                                    <label for="templateNameInput">Tipo de Documento</label>
                                    <input
                                        type="hidden"
            ...
            <div class="col-md- d-none">
                <img src="<?= base_url('assets/images/asistente.png') ?>" width="100%" alt="">
            </div>
        </div>
    </div>
</div>
...
<?= document_orientation_js([
    'auto_rotate' => false, // Mantener orientación original por defecto 
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const titles = ["Subir documento escaneado", "Seleccionar clínica", "Seleccionar tamaño", "Marcar firma", "Nombre del Documento", "Vista previa y descripción"];
        let currentStep = 1;
        ...
        nextBtn.addEventListener('click', async () => {
            ...
            if (currentStep === 6) {
                ...
                // ✅ USAR EL HELPER DE ORIENTACIÓN en lugar de cálculo manual
                let orientation = 'portrait';
                let isHorizontal = '0';
                let rotation = '0';
                let aspectRatio = '1.0';
                
                // Obtener orientación del helper si está disponible
                if (window.documentOrientation) {
                    // ✅ IMPORTANTE: Usar la imagen ORIGINAL guardada, no la rotada
                    ...
                    // Usar los datos del helper
                    orientation = orientationData.orientation || 'portrait';
                    isHorizontal = orientationData.isHorizontal ? '1' : '0';
                    rotation = (window.documentOrientation.imageRotation || 0).toString();
                    aspectRatio = orientationData.aspectRatio || '1.0';
                    
                    console.log('✅ Orientación detectada por helper (imagen original):', orientationData);
                    console.log('✅ Imagen original usada para detección:', originalImage.src.substring(0, 100) + '...');
                    console.log('✅ Fuente de imagen:', window.originalImageForDetection ? 'Guardada' : 'Helper' || 'Fallback');
                } else {
                    // Fallback: cálculo manual solo si el helper no está disponible
                    const imgWidth = image.naturalWidth || image.width;
                    const imgHeight = image.naturalHeight || image.height;
                    const calculatedAspectRatio = imgWidth / imgHeight;
                    
                    if (calculatedAspectRatio > 1.2) {
                        orientation = 'landscape';
                        isHorizontal = '1';
                    }
                    aspectRatio = calculatedAspectRatio.toFixed(2);
                    
                    console.log('⚠️ Helper no disponible, usando cálculo manual');
                }
                
                // ✅ Log detallado de orientación
                console.log('=== ORIENTACIÓN FINAL ===');
                console.log('Orientación:', orientation);
                console.log('¿Es horizontal?', isHorizontal === '1' ? 'SÍ' : 'NO');
                console.log('Rotación:', rotation + '°');
                console.log('Relación ancho/alto:', aspectRatio);
                console.log('Imagen usada para detección:', window.documentOrientation?.originalImage?.src ? 'ORIGINAL' : 'ROTADA');
                console.log('========================');
                ...
                formData.append('template_image', image.src);
                ...
                // ✅ Agregar campos de orientación
                formData.append('orientation', orientation);
                formData.append('is_horizontal', isHorizontal);
                formData.append('rotation', rotation);
                formData.append('aspect_ratio', aspectRatio.toFixed(2));
        ...
        async function uploadFile(file) {
            ...
            try {
                ...
                if (ext === 'pdf') {
                    ...
                    if (window.documentOrientation) {
                        window.documentOrientation.originalImage.src = dataUrl;
                    } else {
                        image.src = dataUrl;
                    }
                }
                // Word
                    ...
                    if (window.documentOrientation) {
                        window.documentOrientation.originalImage.src = d;
                    } else {
                        image.src = d;
                    }
                }
                // Imágenes
                    ...
                    r.onload = () => {
                        if (window.documentOrientation) {
                            window.documentOrientation.originalImage.src = r.result;
                        } else {
                            image.src = r.result;
                        }
                    };
        ...
        // Usar helper para manejar la orientación
        function setupOrientationHandler() {
            if (window.documentOrientation) {
                // Configurar el handler cuando la imagen original se carga
                window.documentOrientation.originalImage.onload = function() {
                    ...
                    // ✅ Guardar referencia a la imagen original para uso posterior
                    window.originalImageForDetection = this;
                    ...
                    setTimeout(() => {
                        ...
                        // ✅ Mostrar información de orientación en el paso 4
                        showOrientationInfo();
                    }, 100);
                    
                    console.log('✅ Orientación procesada con helper:', orientationData);
                    console.log('✅ Imagen original guardada para detección posterior');
                    console.log('✅ Dimensiones originales:', this.naturalWidth + 'x' + this.naturalHeight);
                    console.log('✅ Aspect ratio original:', (this.naturalWidth / this.naturalHeight).toFixed(2));
                };
            }
        }
        
        // Configurar el handler de orientación
        setupOrientationHandler();
        
        // ✅ Función para mostrar información de orientación en el paso 4
        function showOrientationInfo() {
            if (window.documentOrientation) {
                // ✅ IMPORTANTE: Usar la imagen ORIGINAL guardada para mostrar la información correcta
                const originalImage = window.originalImageForDetection || window.documentOrientation.originalImage || image;
                const orientationData = window.documentOrientation.detectOrientation(originalImage);
                const infoDiv = document.getElementById('orientationInfo');
                
                if (infoDiv) {
                    const isHorizontal = orientationData.isHorizontal ? 'landscape' : 'portrait';
                    const rotation = window.documentOrientation.imageRotation || 0;
                    
                    infoDiv.innerHTML = `
                        <i class="fas fa-info-circle"></i> 
                        Documento: <strong>${isHorizontal}</strong> 
                        ${rotation > 0 ? `(Rotado ${rotation}°)` : ''}
                    `;
                    
                    // Mostrar debug si está disponible
                    const debugDiv = document.getElementById('orientationDebug');
                    if (debugDiv) {
                        const debugSpan = document.getElementById('debugOrientation');
                        if (debugSpan) {
                            debugSpan.innerHTML = `
                                <strong>Helper detectó:</strong><br>
                                <strong>Orientación:</strong> ${orientationData.orientation || 'portrait'}<br>
                                <strong>¿Es horizontal?</strong> ${orientationData.isHorizontal ? 'SÍ' : 'NO'}<br>
                                <strong>Relación:</strong> ${orientationData.aspectRatio || 'N/A'}<br>
                                <strong>Rotación aplicada:</strong> ${rotation}°
                            `;
        ...
        // Manejar carga de imagen final (rotada)
        image.onload = () => {
            imageLoaded = true;
            updateCanvas();
            
            // Usar helper para ajustar layout si está disponible
            if (window.documentOrientation && window.documentOrientation.adjustLayout) {
                window.documentOrientation.adjustLayout();
            }
        ...
        canvas.addEventListener('click', function(e) {
            ...
            // ✅ SOLUCIÓN SENCILLA: Normalizar coordenadas para documentos horizontales
            if (window.documentOrientation && window.documentOrientation.isHorizontal) {
                // Para documentos horizontales, normalizar las coordenadas
                ...
                console.log(`Documento horizontal - Coordenadas normalizadas: x=${normalizedX.toFixed(2)}, y=${normalizedY.toFixed(2)}`);
            } else {
                // Para documentos verticales, usar coordenadas estándar
                signatureCoordinates = {
                    x,
                    y
                };
        ...
        // El botón de rotación manual ya está manejado por el helper
        // STEP 2: Clinicas
        let clinics = [];
