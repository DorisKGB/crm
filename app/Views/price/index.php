  ...
  <div class="card">
    ...
    <style>
        ...
        .description-box {
            ...
            border-left: 3px solid #007bff; /* Línea azul a la izquierda */
            background-color: #f9f9f9; /* Fondo gris claro */
            border-radius: 5px; /* Bordes redondeados */
            white-space: pre-wrap; /* Mantiene los saltos de línea */
            max-width: 300px; /* Ajusta el ancho según necesidad */
        }
...
<button type="button" class="btn btn-danger d-none" id="modalTrigger" data-bs-toggle="modal" data-bs-target="#confirmModal">
</button>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-body">
                ¿Deseas eliminar el precio de este examen? Esta acción no se puede deshacer.
            </div>
        ...
        <button type="button" class="btn btn-danger d-none" id="modalTrigger2" data-bs-toggle="modal" data-bs-target="#confirmModal2">
</button>
              <!-- MODAL de confirmación -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmModalLabel">Editar precio de examen</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" id="closeModal2" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body">
                <form id="formEdit" action="">
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label"> <span class="badge-figure"><i class="fas fa-clinic-medical"></i> Clinica</span></label>
                            <input type="text" id="edit_clinic_input" style="border:none;font-size:14px;background-color:#f2f3f0;" readonly class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                        </div>
                    </div>

                    <div class="col-md-12">
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label"> <span class="badge-figure"><i class="fas fa-clinic-medical"></i> Examen</span></label>
                            <input type="text" id="edit_exam" style="border:none;font-size:14px;background-color:#f2f3f0;" readonly class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="exampleInputEmail1" class="form-label"> <span class="badge-figure"><i class="fas fa-text-height"></i> Descripción</span></label>
                            <textarea class="form-control" id="edit_description" rows="5" style="height: auto;border:none;font-size:14px;background-color:#f2f3f0;font-size:12px;" ></textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group mt-2">
                            ...
                            <div class="input-group mb-3">
                            <span class="input-group-text" style="border:none;font-size:20px;" id="basic-addon1">$</span> 
                                <input type="number" id="edit_price" style="border:none;background-color:#f2f3f0;text-align:center;font-size:20px;" class="form-control" aria-describedby="basic-addon1">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group mt-2">
                        <label for="exampleInputEmail1" class="form-label"> <span class="badge-figure"><i class="fas fa-clinic-medical"></i> Estado</span></label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="submit" class="btn btn-primary" onclick="edit()" >Guardar</button>
            </div>
            </form>
            </div>
        </div>
...
<script>
    ...
    $(document).on('dblclick', 'button', function() {
        ...
        console.log("El valor de item_id es: ");
    });                            
    ...
    document.getElementById('clinic_select').addEventListener('change', function(event) {
        ...
        if(!isNaN(selectedValue) && selectedValue !== "" && selectedValue !== null){
            document.getElementById('buttonModalCreate').classList.remove('d-none');
        }else{
            document.getElementById('buttonModalCreate').classList.add('d-none');
        }
    ...
    function ReloadTable(clinic_selected_id){
        ...
        $.ajax({
            url: "<?php echo get_uri('price/listPrice'); ?>",
            type: "GET",
            data: {
    ...
    function edit(){
        ...
        form.addEventListener("submit", function (event){
            ...
            const description = form.edit_description.value;
            const price = form.edit_price.value;
            const state = form.edit_estado.value;


            if(form.edit_price.value == ""){
                return;
            }
            editService(id,price,description,state);
            //form.nameService.value = "";
        });
    }
  

    function editService(id,price,description,state){
        
        modalShow();
        $.ajax({
                url: "<?php echo get_uri('price/editPrice'); ?>",
                type: "GET",
                data: {
                    id: id,
    ...
    function save(){
        ...
        form.addEventListener("submit", function (event){
            ...
            const clinic = form.clinic_id.value;
            const exam = form.exam_id.value;
            ...
            createService(clinic,exam,price,description,user);
            //form.nameService.value = "";
        });
    }

    
    function createService(clinic_id,exam_id,price,description,user){
        
        modalShow();
        $.ajax({
                url: "<?php echo get_uri('price/addPrice'); ?>",
                type: "GET",
                data: {
                    clinic_id: clinic_id,
                    examen_id: exam_id,
    ...
    function drawTable(data) {
        ...
        if (data.length === 0) {
            ...
            cellMessage.setAttribute('colspan', '9'); // Ajustar el colspan al número de columnas
        ...
        data.forEach(function(service) {
            ...
            cellServiceId.innerHTML = '<span><b>'+service.service_name+'</b> </span>';
            ...
            statusCard.setAttribute('item_id',service.id);

            cellState.appendChild(statusCard);
            row.appendChild(cellState);

            // Columna Asignado Por
            var cellAssignedBy = document.createElement('td');
            cellAssignedBy.textContent = service.assigned_by;
            cellAssignedBy.style.fontSize = "12px";
            row.appendChild(cellAssignedBy);
            ...
            buttonDelete.onclick = function () {
                confirmarEliminacion(service.id,service.clinic_id);
            };
            ...
            buttonEdit.classList.add('btn', 'btn-primary','ml-2');
            buttonEdit.innerHTML = '<i class="far fa-edit"></i>';
            buttonEdit.onclick = function () {
                confirmarEdit(service.id,service.clinic_id,service.clinic_name,service.service_name,service.observation,service.price,service.state);
            };
    ...
    function deleteService(id){
            ...
            const form = document.getElementById("formServices");
            $.ajax({
                url: "<?php echo get_uri('price/deletePrice'); ?>",
                type: "GET",
                data: {
                    id: id,
                },
            ...
            function confirmarEliminacion(serviceId,clinic_id) {
            ...
            function confirmarEdit(id,clinic_id,clinic_name,service_name,description,price,state) {
                selectedClinicID = clinic_id;
                document.getElementById("edit_id").value = id;
                document.getElementById('edit_clinic_input').value= clinic_name;
                document.getElementById('edit_exam').value= service_name;
                document.getElementById('edit_description').value= description;
                document.getElementById('edit_price').value= price;
                document.getElementById('edit_estado').value= state;
                document.getElementById('modalTrigger2').click(); // Dispara el modal
            } 
