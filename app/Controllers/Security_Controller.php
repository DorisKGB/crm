...
class Security_Controller extends App_Controller {
    ...
    //only allowed to access for admin users or has admin privileges 
    protected function access_user_module($permission, $condition) {
        if (!($this->login_user->is_admin || get_array_value($this->login_user->permissions, $permission) == $condition)) {
            app_redirect("forbidden");
        }
    }
    ...
    public function can_access_module($menu_item, $check_module = true) {
    ...
    public function can_access_price_create() {
        if (($this->login_user->is_admin || get_array_value($this->login_user->permissions, "can_create_price_list"))) {
            return true;
        }
        return false;
    }

    public function can_access_price_view() {
        if (($this->login_user->is_admin || get_array_value($this->login_user->permissions, "can_see_price_list"))) {
            return true;
        }
        return false;
    }
