...
class Team_members extends Security_Controller {
    ...
    protected $permission_model, $vseeUsers_model;
    protected $Provider_model;

    function __construct() {
        parent::__construct();
        $this->access_only_team_members();
        $this->permission_model = model('App\Models\Permission_model');
        $this->vseeUsers_model = new VseeUsers_model();
         $this->Provider_model = model('App\Models\Provider_model');
    }
    ...
    function branch($user_id) {
        validate_numeric_value($user_id);
        $clinics = $this->permission_model->list_clinic();

        $clinic_data = [];
        foreach ($clinics as $clinic) {
            $values  = $this->permission_model->is_exits_count_table($user_id, $clinic['id']);
            $count = isset($values[0]['count']) ? (int)$values[0]['count'] : 0;
            $used = 1;
            if($count == 0){
                $used = 0;
            }
            
            // Get clockin status for this clinic
            $clockin_status = $this->permission_model->get_clockin_status($user_id, $clinic['id']);
            
            $clinic_data[$clinic['id']] = [
                'id' => $clinic['id'],
                'name' => $clinic['name'], // Un valor adicional (puedes agregar más campos si es necesario)
                'used' => $used, // Otro valor adicional
                'clockin' => $clockin_status, // Clockin status
            ];
        }

        $view_data['clinics'] = $clinic_data;
        $view_data['user_id'] = clean_data($user_id);
        return $this->template->view("team_members/branch/index", $view_data);
    }
    
    function videocall($user_id) {
        validate_numeric_value($user_id);
        //$this->Users_model 
        $vsee = $this->vseeUsers_model->get_one_where(['user_id' => $user_id]);
        $view_data['vsee'] = $vsee;
        $view_data['user_id'] = $user_id;
        return $this->template->view("team_members/videocall/index", $view_data);
    }
    
    function registerVsee($user_id){
        validate_numeric_value($user_id);
        
        $controller = new VseeUsers();
        $controller->run_auto_register_background();
        
        $vsee = $this->vseeUsers_model->get_one_where(['user_id' => $user_id]);
        
        if($vsee && isset($vsee->id)) {
            echo json_encode([
                "success" => true, 
    ...
    function save_branch($user_id) {
        
        validate_numeric_value($user_id);
        $selected_clinics = $this->request->getPost('clinic_list') ?? [];
        $selected_clockin = $this->request->getPost('clockin_list') ?? [];
        $op  = $this->permission_model->delete_data_table($user_id);
        
        foreach ($selected_clinics as $clinic) {
            $values  = $this->permission_model->is_exits_count_table($user_id, $clinic);
                // Acceder al valor de 'count' en el array
            $count = isset($values[0]['count']) ? (int)$values[0]['count'] : 0;
            if($count == 0){
                $this->permission_model->delete(5);
                $data = [
                    'id_user' => $user_id,
                    'id_clinic' => $clinic,
                    'clockin' => in_array($clinic, $selected_clockin) ? 1 : 0
                ];
                $result = $this->permission_model->ci_save($data);
            }
        }
        ...
        return redirect()->to('/team_members/view/'.$user_id.'/branch');
    }

    //show provider information of a team member
    function provider($user_id) {
        validate_numeric_value($user_id);
        $this->update_only_allowed_members($user_id);
        ...
        $provider_info = $this->Provider_model->get_by_user_id($user_id);
        $view_data['provider_info'] = $provider_info;
        $view_data['has_provider_credentials'] = $provider_info ? true : false;

        return $this->template->view("team_members/provider", $view_data);
    }

    //save provider credentials
    function save_provider_credentials($user_id) {
        validate_numeric_value($user_id);
        $this->update_only_allowed_members($user_id);
        ...
        $existing_provider = $this->Provider_model->get_by_user_id($user_id);
        ...
        $remove_signature = $this->request->getPost('remove_signature');
        
        if ($signature_file && $signature_file->isValid() && !$signature_file->hasMoved()) {
            $upload_path = FCPATH . 'writable/firmas/';
            ...
            $file_info = pathinfo($signature_file->getName());
            $signature_filename = $user_info->first_name . '_' . $user_info->last_name . '_' . time() . '.' . $file_info['extension'];
            $signature_filename = strtolower(str_replace(' ', '_', $signature_filename));
            
            if ($signature_file->move($upload_path, $signature_filename)) {
                // Delete old signature file if exists
                if ($existing_provider && $existing_provider->url_signature) {
                    $old_file = $upload_path . $existing_provider->url_signature;
                    if (file_exists($old_file)) {
                        unlink($old_file);
                    }
                }
            } else {
                echo json_encode(array("success" => false, 'message' => app_lang('signature_upload_failed')));
                return;
            }
        } else if ($remove_signature == '1') {
            // User wants to remove signature
            if ($existing_provider && $existing_provider->url_signature) {
                $upload_path = FCPATH . 'writable/firmas/';
                $old_file = $upload_path . $existing_provider->url_signature;
                if (file_exists($old_file)) {
            ...
            $signature_filename = ''; // Empty signature
        } else {
            // Keep existing signature if no new file uploaded and not removing
            if ($existing_provider && $existing_provider->url_signature) {
                $signature_filename = $existing_provider->url_signature;
            }
        }
        ...
        $provider_data = array(
            ...
            "role" => $this->request->getPost('role'),
            "url_signature" => $signature_filename,
            "deleted" => 0
        );

        $provider_data = clean_data($provider_data);

        if ($existing_provider) {
            // Update existing provider
            $provider_updated = $this->Provider_model->ci_save($provider_data, $existing_provider->id);
        } else {
            // Create new provider
            $provider_updated = $this->Provider_model->ci_save($provider_data);
        }
    ...
    function delete_provider_credentials($user_id) {
        validate_numeric_value($user_id);
        $this->update_only_allowed_members($user_id);
        ...
        if ($provider_info) {
            // Delete physical signature file if exists
            if ($provider_info->url_signature) {
                $upload_path = FCPATH . 'writable/firmas/';
                $signature_file = $upload_path . $provider_info->url_signature;
            ...
            if ($this->Provider_model->ci_save($delete_data, $provider_info->id)) {
                echo json_encode(array("success" => true, 'message' => app_lang('record_deleted')));
            } else {
                echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
            }
