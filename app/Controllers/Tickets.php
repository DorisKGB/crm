...
class Tickets extends Security_Controller {
    ...
    {
        $clinics = $this->Ticket_templates_model->getClinics2($user_id);
        $clinic_options = [];
    ...
    protected function getClinicsDropdown($user_id)
    {
        $clinics = $this->Ticket_templates_model->getClinics2($user_id); // Obtén las clínicas
        $clinic_dropdown = array(array("id" => "", "text" => "- " . app_lang("clinic") . " -"));

        foreach ($clinics as $clinic) {
            $clinic_dropdown[] = array("id" => $clinic->id, "text" => $clinic->name);
        }
    ...
    function index($status = "", $ticket_type_id = 0) {
        ...
        //$view_data['clinics'] = json_encode($this->getClinicOptions2($this->login_user->id)); 
        //var_dump($this->getClinicsDropdown($this->login_user->id));
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            //_get_clinic_dropdown
            $view_data['clients_dropdown'] = json_encode($this->_get_clinic_dropdown());
    ...
    function modal_form() {
        ...
        $view_data['show_project_reference'] = get_setting('project_reference_in_tickets');
        $view_data['clinic_dropdown'] = $this->getClinicOptions2($this->login_user->id); 
    ...
    function save() {
        ...
        //echo json_encode(array("success" => false, 'data' => $ticket_data ,'message' => app_lang('error_occurred')));
        ...
        if ($ticket_id) {
            ...
            echo json_encode(array("success" => false, 'data' => $client_id ,'message' => app_lang('error_occurred')));
        }
    }
    ...
    function update_comment(){
        ...
        $response = $this->Ticket_comments_model->update_ticket_comment($id,$comment);
        if($response){
    ...
    function save_comment() {
        ...
        if($is_comment == 0){
            ...
            $comment_id = $is_comment; // Usamos el ID del comentario que vino desde is_comment
            $comment_data = array(
                "description" => $description,
                "ticket_id" => $ticket_id,
                "files" => $files_data,
            ...
            $this->Ticket_comments_model->ci_save($comment_data, $comment_id);
            //$message = app_lang('comment_updated');
        }
    ...
    private function _get_clinic_dropdown(){
        
        $clinic_dropdown = array(array("id" => "", "text" => "- " . app_lang("clinic") . " -"));
        $clients_list = $this->getClinicOptions2($this->login_user->id);
        foreach ($clients_list as $id => $value) {
            $clinic_dropdown[] = array("id" => $id, "text" => $value);
        }
        /*foreach ($clients_list as $key => $value) {
            $clinic_dropdown[] = array("id" => $key, "text" => $value);
        }*/
        return $clinic_dropdown;
    }
