...
class Webhooks_listener extends App_Controller
{
    ...
    public function crosschex_attendance($key)
    {
         $inserted = 0;
        try {
            //log_message('success', 'Nuevo dato');
            helper('date_time_helper');
            
            // 1) Leer cabeceras y cuerpo
            $headers  = getallheaders();
            $rawBody  = file_get_contents('php://input');
            $payload  = json_decode($rawBody, true);

             // Logging para depuración
            log_message('debug', '=== Webhook CrossChex Llegó ===');
            log_message('debug', 'Headers: ' . print_r($headers, true));
            log_message('debug', 'Body: '    . $rawBody);

            // 2) Validar clave simple
            $settingsKey = get_setting('crosschex_webhook_key');
            if (!$settingsKey || $settingsKey !== $key) {
                log_message('error', 'Webhook CrossChex: clave inválida');
                echo json_encode([
                    'code' => '403',
                    'msg'  => 'Clave inválida'
                ]);
            ...
            // 2) Normalizar claves a minúsculas
            $h = array_change_key_case($headers, CASE_LOWER);

            // 3) Validar que sea el evento que nos interesa
            $ns     = $h['namespace']   ?? '';
            $action = $h['nameaction']  ?? '';
            if (strtolower($ns) !== 'attendance.record' || strtolower($action) !== 'realrecord') {
                log_message('debug', "Webhook CrossChex: cabeceras no coinciden ns={$ns} action={$action}");
                // No es un realrecord: devolver 204 para que no lo reintente
                echo json_encode([
                    'code' => '204',
                    'msg'  => 'Evento no procesado'
                ]);
                return;
            }

            // 4) Extraer la lista de registros
            $list = $payload['payload']['list'] ?? [];
            if (! is_array($list)) {
                log_message('error', 'Webhook CrossChex: payload.list no es un array');
                echo json_encode([
                    'code' => '400',
            ...
            $model    = new \App\Models\Clock_in_model();
            $inserted = 0;

            foreach ($list as $entry) {
                $workno    = $entry['employee']['workno']    ?? null;
                $checktime = $entry['checktime']             ?? null;
                $clinicId  = $entry['device']['clinic_id']   ?? null;

                if (! $workno || ! $checktime || ! $clinicId) {
                    continue;
                }

                // 5) Convertir UTC → local (usa tu helper)
                $localDt = convert_date_utc_to_local($checktime);
                $date    = get_date_from_datetime($localDt);
                $time    = get_time_from_datetime($localDt);

                // 6) Evitar duplicados
                $exists = $model->get_one_where([
                    'user_id'   => $workno,
                    'clinic_id' => $clinicId,
                    'date'      => $date,
                    'time'      => $time,
                ...
                $dataToInsert = [
                    ...
                    'date'      => $date,
                    'time'      => $time,
                    'remark'    => 'Importado vía Webhook CrossChex'
                ];

                // 7) Insertar
                $model->ci_save($dataToInsert);
                $inserted++;
            }

            // 8) Responder OK para que CrossChex deje de reenviar
            ...
            log_message('error', 'Webhook CrossChex error: ' . $e->getMessage());
            echo json_encode([
                'code' => '500',
