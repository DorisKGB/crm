...
class Signin extends App_Controller {
    ...
    // Aquí es donde debes agregar el código para marcar al usuario como logueado
        $session = session();
        $session->set('isLoggedIn', true); // Se establece la sesión como 'logueado'
    ...
    function sign_out() {
        // Obtener el ID del usuario antes de cerrar la sesión
        $user_id = $this->Users_model->login_user_id();
        
        // Eliminar el token de notificación del usuario si existe
        if ($user_id) {
            $notification_token_model = new \App\Models\Notification_Token_Model();
            $notification_token_model->deleteUserToken($user_id);
        }
