...
class Dashboard extends Security_Controller {
    ...
    protected $roles_model_table;
    protected $setting_model_table;
    protected $users_model;

    function __construct() {
        parent::__construct();
        $this->Custom_widgets_model = model('App\Models\Custom_widgets_model');
        $this->roles_model_table = new Roles_model();
        $this->setting_model_table = new Settings_model();
    ...
    public function index($default = 0) {
        ...
        $panel_id = $this->get_role_by_panel_or_by_role($this->login_user->role_id,false);//panel
        $this->Settings_model->save_setting("user_" . $this->login_user->id . "_dashboard", $panel_id, "user");
        ...
        if ($this->login_user->user_type === "staff" && $this->show_staff_on_staff) {
            ...
            $selected_dashboard_id = get_setting("user_" . $this->login_user->id . "_dashboard"); //toma el valor de 10
    ...
    function view($id = 0) { //Esta es la funcion VIEW

        validate_numeric_value($id);

        $id_new = $this->get_role_by_panel_or_by_role($this->login_user->role_id,false);
        if($id_new != null){
            $id = $id_new;
            $this->Settings_model->save_setting("user_" . $this->login_user->id . "_dashboard", $id, "user");
        }
        ...
        //todos los roles guardado
        $roles_exists = $this->get_roles_from_file();

        if($this->login_user->is_admin || !in_array($this->login_user->role_id, $roles_exists)){
            $dashboard_info = $this->_get_my_dashboard($id, $this->is_staff_dashboard($id));
        }else{
            $dashboard_info = $this->_get_my_dashboard($id, true);
        }
        ...
        if ($dashboard_info) {
            ...
            $roles = $this->roles_model_table->get_all_roles(); 
            $role_options = [];
            foreach ($roles as $role) {
                $role_options[$role->id] = $role->title; // Asumiendo que las columnas son 'id' y 'name'
            }
            $view_data['role_options'] = $roles;
            $get_role_by_panel = $this->get_role_by_panel_or_by_role($id,true);
            $view_data['role_select'] = ($get_role_by_panel == "") ? "" : $get_role_by_panel;
    ...
    function delete() {
        ...
        if ($this->_get_my_dashboard($id) && $this->Dashboards_model->delete($id)) {
            ...
            //Aqui elimina el panel
            $this->delete_lines_by_panel();
    ...
    public function save_custom_widget_role()
    {
        // Cargar el helper 'file' para utilizar write_file()
        helper('filesystem');

        // Obtén los parámetros del POST
        $id = $this->request->getPost('panel_id');
        $role_select = $this->request->getPost('role_id');

        // Nombre constante del archivo
        $fileName = WRITEPATH . 'logs/role_panel_log.txt'; // Archivo único
        ...
        if (file_exists($fileName)) {
            ...
            // Si el rol es 0, eliminar la línea correspondiente al panel
            if ((int)$role_select === 0) {
                $updatedLines = [];
                $role_active = "";

                foreach ($lines as $line) {
                    // Verificar si la línea contiene el panel actual
                    if (preg_match('/\["rol = (\d+)", "panel = (\d+)"\]/', $line, $matches)) {
                        $line_role_id = (int)$matches[1];
                        $line_panel_id = (int)$matches[2];
                        if ($line_panel_id === (int)$id) {
                            // No incluir esta línea en la lista actualizada
                            $role_active = $line_role_id;
                            continue;
                        }
                    }
                    $updatedLines[] = $line;
                ...
                file_put_contents($fileName, implode(PHP_EOL, $updatedLines) . PHP_EOL);

                //Necesito quitarlo del setting
                //$this->Settings_model->save_setting("user_" . $this->login_user->id . "_dashboard", "", "user");
                if($role_active != ""){
                    $users_clean = $this->users_model->get_users_ids_by_role($role_active);
                // Verificar si se obtuvieron usuarios
                    if (!empty($users_clean)) {
                        foreach ($users_clean as $user) {
                            // $user será un array, no un objeto, por lo que debes acceder usando la clave
                            $this->Settings_model->save_setting("user_" . $user['id'] . "_dashboard", "", "user");
                        }
                    }
                }
                ...
                // Si el rol no está presente en el archivo, agregarlo
                $found = false;
                foreach ($lines as &$line) {
                    if (preg_match('/\["rol = (\d+)", "panel = (\d+)"\]/', $line, $matches)) {
                        $line_role_id = (int)$matches[1];
                        ...
                        // Si el rol ya existe, actualizar el panel
                        if ($line_role_id === (int)$role_select) {
                            $line = '["rol = ' . $role_select . '", "panel = ' . $id . '"]';
                            $found = true;
                            break;
                        }
                    }
                ...
                if (!$found) {
                    // Si el rol no existe, agregar una nueva línea
                    $lines[] = '["rol = ' . $role_select . '", "panel = ' . $id . '"]';
                }
                
                // Reescribir el archivo con las líneas actualizadas
                file_put_contents($fileName, implode(PHP_EOL, $lines) . PHP_EOL);
                //echo "Rol y panel actualizados en el archivo.";

                $users_clean = $this->users_model->get_users_ids_by_role($role_select);
                foreach ($users_clean as $user) {
                    $this->Settings_model->save_setting("user_" . $user['id'] . "_dashboard", $id, "user");
                }
            }
        } else {
            // Si el archivo no existe, crear y agregar el rol y el panel
            if (!is_dir(WRITEPATH . 'logs')) {
                mkdir(WRITEPATH . 'logs', 0777, true); // Crear el directorio si no existe
            }

            $dataToSave = '["rol = ' . $role_select . '", "panel = ' . $id . '"]' . PHP_EOL;
            write_file($fileName, $dataToSave);
            echo "Archivo creado y rol agregado.";
        }

        app_redirect("dashboard/view/" . $id);
    }



    public function get_roles_from_file()
    {
        helper('filesystem');
        // Ruta del archivo
        $fileName = WRITEPATH . 'logs/role_panel_log.txt';
        ...
        // Inicializar un array para almacenar los roles
        $roles = [];

        // Usar una expresión regular para buscar los valores de "rol = X"
        preg_match_all('/rol = (\d+)/', $fileContent, $matches);

        // Si se encontraron coincidencias, agregar al array
        if (!empty($matches[1])) {
            $roles = array_map('intval', $matches[1]); // Convertir a enteros
        }

        // Retornar el array de roles únicos
        return array_unique($roles);
    }


    function get_role_by_panel_or_by_role($id,$is_panel_or_role) {
        helper('filesystem');
        // Ruta al archivo
        $file_path = WRITEPATH . 'logs/role_panel_log.txt';
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/\["rol = (\d+)", "panel = (\d+)"\]/', $line, $matches)) {
                $role_id = (int)$matches[1];  // Capturar el ID del rol
                $line_panel_id = (int)$matches[2]; // Capturar el ID del panel
    
                // Si el panel coincide con el que buscamos, retornamos el rol
                if($is_panel_or_role){ //me devuelve el rol
                    if ($line_panel_id === (int)$id) {
                        return $role_id;
                    }
                }else{
                    if ($role_id === (int)$id) {
                        return $line_panel_id;
                    }
                }
    ...
    function delete_lines_by_panel($panel_id)
    {
        helper('filesystem');
        ...
        // Inicializar un array para almacenar las líneas actualizadas
        $updated_lines = [];

        // Recorrer las líneas del archivo
        foreach ($lines as $line) {
            // Validar si la línea contiene el panel_id
            if (preg_match('/\["rol = (\d+)", "panel = (\d+)"\]/', $line, $matches)) {
                $line_panel_id = (int)$matches[2];

                // Si el panel_id coincide, no incluir esta línea en el archivo actualizado
                if ($line_panel_id === (int)$panel_id) {
                    continue;
                }
            }

            // Agregar la línea al array actualizado
            $updated_lines[] = $line;
        }
        ...
        echo "Líneas con panel_id = $panel_id eliminadas correctamente.";
        return true;
    }
