...
class Users_model extends Crud_model {
    ...
    function get_id_roles_for($id_role) {
    ...
    function get_user_by_id($id) {
        ...
        // Consulta para obtener el registro del usuario por ID
        $sql = "SELECT * FROM $users_table WHERE $users_table.deleted = 0 AND $users_table.id = '$id';";
        ...
        if ($result->getNumRows() > 0) {
            return $result->getRowArray(); // Retorna los datos del usuario
        } else {
    ...
    function update_last_activity($user_id) {
        try {
            ...
            log_message('error', 'Error actualizando última actividad del usuario: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Verificar si un usuario está online (última actividad en los últimos 5 minutos)
     */
    function is_user_online($user_id) {
        try {
            ...
            $sql = "SELECT COUNT(*) as count 
                    ...
                    AND TIMESTAMPDIFF(MINUTE, last_online, ?) <= 5";
            
            $result = $this->db->query($sql, [$user_id, $current_time]);
            $row = $result->getRow();
            
            return $row && $row->count > 0;
            
        } catch (\Exception $e) {
            log_message('error', 'Error verificando estado online del usuario: ' . $e->getMessage());
            return false;
        }
    }


    function get_users_ids_by_role($role_id) {
        $users_table = $this->db->prefixTable('users');
    
        $sql = "SELECT $users_table.id
                FROM $users_table
                WHERE $users_table.deleted = 0 AND $users_table.role_id = $role_id";
        
        // Ejecutar la consulta y devolver los resultados como un array de IDs
