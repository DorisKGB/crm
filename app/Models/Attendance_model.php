...
class Attendance_model extends Crud_model {
    ...
    public function findApprovedByUserOnDate($userKey, string $date)
    {
        $start = $date . ' 00:00:00';
        $end   = $date . ' 23:59:59';

        // 1) Nuevo builder limpio
        $builder = $this->db->table($this->table);

        // 2) Tus condiciones
        $builder->where('status',    'incomplete');
        $builder->where('user_id',   $userKey);
        $builder->where('in_time >=', $start);
        $builder->where('in_time <=', $end);

        // 3) Sólo un registro
        $query = $builder->get(1);

        // 4) Devuelvo objeto o null si no hay nada
        return $query->getRow();
    }
