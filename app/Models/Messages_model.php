...
class Messages_model extends Crud_model {
    ...
    function get_chat_list($options = array()) {
        ...
        /*$sql = "SELECT $messages_table.id, $messages_table.subject, $messages_table.from_user_id, 
        IF(another_m.mex_created_at, another_m.mex_created_at, $messages_table.created_at) AS message_time, 
        IF(another_m.status, another_m.status, $messages_table.status) AS status, 
        (SELECT from_user_id FROM $messages_table WHERE $messages_table.id=another_m.max_id) AS last_from_user_id,
        CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name, 
        $users_table.image AS user_image, $users_table.last_online,
        (SELECT COUNT(m2.id) FROM $messages_table m2 
         WHERE m2.deleted=0 AND m2.status='unread' AND m2.to_user_id=$login_user_id 
         AND (m2.message_id=$messages_table.id OR m2.id=$messages_table.id)) AS unread_count
        FROM $messages_table
        LEFT JOIN (SELECT MAX(id) as max_id, MAX(message_id) as mex_message_id, MAX(created_at) as mex_created_at, MAX(status) as status FROM $messages_table WHERE deleted=0 and  message_id!=0 GROUP BY message_id) AS another_m ON $messages_table.id=another_m.mex_message_id
        LEFT JOIN $users_table ON ($users_table.id=$messages_table.from_user_id OR $users_table.id=$messages_table.to_user_id) AND $users_table.id != $login_user_id
        WHERE $messages_table.deleted=0 AND $messages_table.message_id=0 $where AND
        ...
        $users_table.image AS user_image, $users_table.last_online,
        (SELECT COUNT(*) 
         FROM $messages_table unread_m 
         WHERE unread_m.deleted=0 AND unread_m.status='unread' 
         AND unread_m.to_user_id=$login_user_id 
         AND (unread_m.message_id=$messages_table.id OR 
              (unread_m.id=$messages_table.id AND $messages_table.message_id=0))) AS unread_count
        FROM $messages_table
        LEFT JOIN (SELECT MAX(id) as max_id, MAX(message_id) as mex_message_id, MAX(created_at) as mex_created_at, MAX(status) as status 
                  FROM $messages_table WHERE deleted=0 and message_id!=0 GROUP BY message_id) AS another_m 
                  ON $messages_table.id=another_m.mex_message_id
        LEFT JOIN $users_table ON ($users_table.id=$messages_table.from_user_id OR $users_table.id=$messages_table.to_user_id) 
                 AND $users_table.id != $login_user_id
        WHERE $messages_table.deleted=0 AND $messages_table.message_id=0 $where AND
        FIND_IN_SET($login_user_id, $messages_table.deleted_by_users) = 0 
        AND ($messages_table.from_user_id=$login_user_id OR $messages_table.to_user_id=$login_user_id)
    ...
    function count_notifications($user_id, $last_message_checke_at = "0", $active_message_id = 0, $user_ids = "") {
        ...
        // Usar una consulta más compatible con diferentes versiones de MySQL
        $sql = "SELECT COUNT($messages_table.id) AS total_notifications
        FROM $messages_table
        WHERE $messages_table.deleted=0 
        AND $messages_table.status='unread'  
        AND $messages_table.to_user_id = $user_id
        AND $messages_table.created_at > '$last_message_checke_at' $where";

        try {
            $result = $this->db->query($sql);
            if ($result && $result->resultID->num_rows) {
                return $result->getRow()->total_notifications;
            }
        } catch (\Exception $e) {
            // Log del error y retornar 0
            log_message('error', 'Error en count_notifications: ' . $e->getMessage());
            error_log('ERROR en count_notifications: ' . $e->getMessage());
        }
        
        return 0; // Retornar 0 si no hay resultados o hay error
    }
    ...
    function count_unread_messages_for_chat($message_id, $user_id) {
        $messages_table = $this->db->prefixTable('messages');
        ...
        $sql = "SELECT COUNT($messages_table.id) as total
                ...
                AND $messages_table.status = 'unread'  
                AND $messages_table.to_user_id = $user_id 
                AND ($messages_table.message_id = $message_id OR $messages_table.id = $message_id)";
        
        return $this->db->query($sql)->getRow()->total;
    }
    
    function count_unread_messages_from_user($from_user_id, $to_user_id) {
        $messages_table = $this->db->prefixTable('messages');
        ...
        $sql = "SELECT COUNT($messages_table.id) as total
                ...
                WHERE $messages_table.deleted = 0 
                AND $messages_table.status = 'unread'  
                AND $messages_table.from_user_id = $from_user_id 
                AND $messages_table.to_user_id = $to_user_id";
        
        return $this->db->query($sql)->getRow()->total;
    }
    public function get_unread_counts_by_user(int $user_id): array
    {
        $messages = $this->db->prefixTable('messages');
    
        $sql = "SELECT from_user_id, COUNT(*) AS unread_count
                FROM $messages
                WHERE deleted = 0
                  AND status = 'unread'
                  AND to_user_id = ?
                  AND FIND_IN_SET(?, $messages.deleted_by_users) = 0
                GROUP BY from_user_id";
    
        $result = $this->db->query($sql, [$user_id, $user_id])->getResultArray();
    
        $map = [];
        foreach ($result as $row) {
            $map[(string)$row['from_user_id']] = (int)$row['unread_count'];
        }
        return $map;
    }
    
    public function get_unread_counts_by_threads(int $user_id, array $message_ids): array
    {
        ...
        // Usamos el "thread id" real: si message_id=0, el thread es el propio id (mensaje raíz)
        // Contamos solo mensajes no leídos dirigidos al usuario y no "borrados para el usuario"
        $builder = $this->db->table($messages);
        $builder->select("IF($messages.message_id = 0, $messages.id, $messages.message_id) AS thread_id", false);
        $builder->select("COUNT(*) AS unread_count", false);
        $builder->where("$messages.deleted", 0);
        $builder->where("$messages.status", 'unread');
        $builder->where("$messages.to_user_id", $user_id);
    
        // Respetar "borrado para el usuario" si lo usas en el inbox
        $builder->where("FIND_IN_SET($user_id, $messages.deleted_by_users) = 0", null, false);
    
        // Incluir mensajes cuya raíz esté en la lista (id) o respuestas que pertenezcan a esa raíz (message_id)
        $builder->groupStart()
                ->whereIn("$messages.message_id", $message_ids)
                ->orWhereIn("$messages.id", $message_ids)
                ->groupEnd();
        ...
        $out = [];
        foreach ($rows as $r) {
            $out[(string)$r['thread_id']] = (int)$r['unread_count'];
        }
        return $out;
    }

    function get_recently_edited_messages($thread_id, $since_datetime, $user_id) {
        $messages_table = $this->db->prefixTable('messages');
        
        $sql = "SELECT id, message FROM $messages_table 
        ...
        return $this->db->query($sql, array($thread_id, $thread_id, $since_datetime, $user_id, $user_id))->getResult();
    }
}
