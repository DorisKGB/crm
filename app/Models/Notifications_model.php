...
class Notifications_model extends Crud_model
{
    ...
    function create_notification($event, $user_id, $options = array())
    {
        ...
        $stamp_id = get_array_value($options, "stamp_id");
        ...
        $notify_to_admins_only = get_array_value($options, "notify_to_admins_only"); //Aqui notificar solo admin
        ...
        $daily_report_id = get_array_value($options, "daily_report_id");
        ...
        $data = array(
            ...
            "daily_report_id" => $daily_report_id ? $daily_report_id : ""
        );
    ...
    function get_notifications($user_id, $offset = 0, $limit = 20)
    {
        ...
        $report_table = $this->db->prefixTable('daily_reports');
        $stamp_table = $this->db->prefixTable('stamp');
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $notifications_table.*, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name, $users_table.image AS user_image,
                 ...
                 $report_table.clinic_name AS clinic_title,
                 $stamp_table.clinic_select AS clinic_select,
        ...
        LEFT JOIN $report_table ON $report_table.id=$notifications_table.daily_report_id
        LEFT JOIN $stamp_table ON $stamp_table.id=$notifications_table.stamp_id
    ...
    function crearCadena(array $numeros): string
    {
        return implode(",", $numeros);
    }

    public function create_simple_notification(int $user_id, string $event, array $users, array $entities = [])
    {
        // Lista de todas las columnas “entidad” que maneja tu tabla
        $entityColumns = [
            'project_id',
            'task_id',
            'project_comment_id',
        ...
        $data = [
            ...
            'description' => '',
            'created_at'  => $now,
            'notify_to'   => implode(',', $users),
            'read_by'     => '',
            'event'       => $event,
            'deleted'     => 0,
        ];

        // Rellenar dinámicamente cada columna de entidad
        foreach ($entityColumns as $col) {
            // Si viene en $entities lo usamos; si no, cadena vacía
            $data[$col] = isset($entities[$col]) ? $entities[$col] : '';
        }

        return $this->ci_save($data);
    }

    function create_notification_daily_reports($user_id, $daily_report_id, $users)
    {
        $users_notify = $this->crearCadena($users);
        $notifications_table = $this->db->prefixTable('notifications');
        $data["user_id"] = $user_id;
        $data["description"] = "";
        $data["created_at"] = date("Y-m-d H:i:s");
        $data["notify_to"] = $users_notify;
        $data["read_by"] = "";
        $data["event"] = "daily_report_success";
        $data["project_id"] = "";
        $data["task_id"] = "";
        $data["project_comment_id"] = "";
        $data["ticket_id"] = "";
        $data["ticket_comment_id"] = "";
        ...
        $data["contract_id"] = "";
        $data["order_id"] = "";
        $data["estimate_request_id"] = "";
        $data["actual_message_id"] = "";
        $data["parent_message_id"] = "";
        $data["event_id"] = "";
        $data["stamp_id"] = "";
        $data["announcement_id"] = "";
        $data["proposal_id"] = "";
        $data["estimate_comment_id"] = "";
        $data["subscription_id"] = "";
        $data["expense_id"] = "";
        $data["proposal_comment_id"] = "";
        $data["daily_report_id"] = $daily_report_id;
        $data["deleted"] = "0";
        $this->ci_save($data);
    }

    function create_notification_stamp($user_id, $stamp_id, $users)
    {
        $users_notify = $this->crearCadena($users);
        $notifications_table = $this->db->prefixTable('notifications');
        $data["user_id"] = $user_id;
