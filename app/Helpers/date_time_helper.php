...
if (! function_exists('get_hour_minute_from_datetime')) {
    /**
     * Dado un datetime (string o DateTime), devuelve "HH:MM"
     *
     * @param  string|\DateTime  $dateTime
     * @return string            Hora y minuto en formato 24h, o cadena vacía si es inválido
     */
    function get_hour_minute_from_datetime($dateTime): string
    {
        try {
            if (! $dateTime) {
                return '';
            ...
            // Aceptar tanto DateTime como string
            ...
            log_message('error', 'get_hour_minute_from_datetime: ' . $e->getMessage());
            return '';
        }
    }
}
...
if (!function_exists('convert_date_local_to_utc')) {
    ...
    function convert_date_local_to_utc($date = "", $format = "Y-m-d H:i:s") {
        ...
        $localTimezone = get_setting("timezone");
        $localDate = new DateTime($date, new DateTimeZone($localTimezone));
...
if (!function_exists('get_current_utc_time')) {
    /*function get_current_utc_time($format = "Y-m-d H:i:s") {
        ...
        // Si no se encuentra una zona horaria válida, establecer UTC por defecto
        if (empty($timezone)) {
            $timezone = 'UTC';
        }

        // Crear una nueva instancia de DateTime usando la zona horaria del servidor (local)
        $date = new DateTime("now", new DateTimeZone($timezone));  // Usa la zona horaria configurada
        
        // Devolver la fecha en el formato especificado
        return $date->format($format);
    }*/
    function get_current_utc_time($format = "Y-m-d H:i:s") {
        // Siempre crear en UTC
...
if (!function_exists('convert_date')) {
    function convert_date($date_time, $format = "Y-m-d H:i:s", $timezone = "America/Chicago") {
        try {
            $date = new DateTime($date_time, new DateTimeZone("UTC")); 
            $date->setTimezone(new DateTimeZone($timezone)); 
            return $date->format($format);
        } catch (Exception $e) {
            // En caso de que el timezone no sea válido
...
if (!function_exists('convert_time_or_dash')) {

    function convert_time_or_dash($time = "")
    {
        if (!$time || $time === '--') {
            return '--';
        }
        ...
        $check_time = explode(":", $time);
        $hour = is_numeric($check_time[0]) ? (int)$check_time[0] : 0;
        $minute = isset($check_time[1]) && is_numeric($check_time[1]) ? str_pad($check_time[1], 2, "0", STR_PAD_LEFT) : "00";
        $second = isset($check_time[2]) && is_numeric($check_time[2]) ? ":" . str_pad($check_time[2], 2, "0", STR_PAD_LEFT) : ":00";
        ...
        if ($hour == 0) {
            ...
            return str_pad($hour - 12, 2, "0", STR_PAD_LEFT) . ":$minute$second PM";
        } else {
            return str_pad($hour, 2, "0", STR_PAD_LEFT) . ":$minute$second AM";
        }
    }
}
...
if (!function_exists('format_to_relative_time_2')) {

    function format_to_relative_time_2($date_time, $convert_to_local = true, $is_24_hour_format = true) {
        // Recuperamos la zona horaria configurada
        $timezone = get_setting('timezone');
        if (empty($timezone)) {
            $timezone = 'America/New_York'; // Usa una zona horaria por defecto
        }

        // Si la fecha está en UTC, la convertimos a la zona horaria local si es necesario
        if ($convert_to_local) {
            $date_time = convert_date_utc_to_local($date_time); // Convierte de UTC a la zona horaria local si es necesario
        }

        // Crear un objeto DateTime para la fecha de entrada
        $target_date = new DateTime($date_time);
        $now = new DateTime();
        $now->setTimezone(new DateTimeZone($timezone)); // Ajusta a la zona horaria local
        ...
        if ($now->format("Y-m-d") == $target_date->format("Y-m-d")) {
            // Si es hoy, formatear la hora como "Hoy a las 5 PM"
            $time_format = $is_24_hour_format ? "H:i" : "h:i A"; // Determinar si el formato es 24 horas o 12 horas
            return app_lang("today_at") . " " . $target_date->format($time_format);
        }
        // Verificar si la fecha es ayer
        else if ($yesterday == $target_date->format("Y-m-d")) {
            // Si es ayer, formatear la hora como "Ayer a las 5 PM"
            $time_format = $is_24_hour_format ? "H:i" : "h:i A"; // Determinar si el formato es 24 horas o 12 horas
            return app_lang("yesterday_at") . " " . $target_date->format($time_format);
        } else {
            // Si no es hoy ni ayer, devolver la fecha completa
            return $target_date->format("Y-m-d H:i");
        }
    }
}
...
if (!function_exists('format_date_to_spanish_long')) {
    function format_date_to_spanish_long($date = "") {
