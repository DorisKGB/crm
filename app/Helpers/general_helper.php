...
if (!function_exists('decode2_id')) {



    function decode2_id($id, $salt)
 ...
 */
 ...
 if (!function_exists('simple_notification')) {
    /**
     * Envía una notificación genérica usando Notifications_model::create_simple_notification()
     *
     * @param int    $user_id    ID del usuario que origina la notificación
     * @param string $event      Clave del evento en crm_notification_settings
     * @param array  $user_ids   Lista de IDs de usuarios a notificar
     * @param array  $entities   Columnas de entidad (clave=>valor) p.ej. ['stamp_id'=>123]
     * @return int|null          ID de la notificación o null si falla
     */
    function simple_notification(int $user_id, string $event, array $user_ids, array $entities = [])
    {
        // Cargamos el modelo
        $model = new Notifications_model();
        return $model->create_simple_notification($user_id, $event, $user_ids, $entities);
    }
}
...
if (!function_exists('get_logosignin_url')) {
    function get_logosignin_url()
    {
        // Cambia esta ruta a la ubicación de tu imagen
        return get_file_from_setting("signin_logo");
    }
}
...
/**
 * Verificar si el usuario puede administrar el módulo Clock-in
 * @return bool
 */
if (!function_exists('can_admin_clockin')) {
    function can_admin_clockin()
    {
        $ci = new Security_Controller(false);
        
        // Los administradores del sistema pueden administrar todo
        if ($ci->login_user->is_admin) {
            return true;
        }
        
        $clock_permission = get_array_value($ci->login_user->permissions, 'clock_permission');
        return $clock_permission === 'admin';
    }
}

/**
 * Verificar si el usuario puede ver por clínicas el módulo Clock-in
 * @return bool
 */
if (!function_exists('can_view_clinic_clockin')) {
    function can_view_clinic_clockin()
    {
        $ci = new Security_Controller(false);
        
        // Los administradores del sistema pueden ver todo
        if ($ci->login_user->is_admin) {
            return true;
        }
        
        $clock_permission = get_array_value($ci->login_user->permissions, 'clock_permission');
        return $clock_permission === 'clinic' || $clock_permission === 'admin';
    }
}

/**
 * Verificar si el usuario puede ver su propio marcaje en Clock-in
 * @return bool
 */
if (!function_exists('can_view_own_clockin')) {
    function can_view_own_clockin()
    {
        $ci = new Security_Controller(false);
        ...
        $clock_permission = get_array_value($ci->login_user->permissions, 'clock_permission');
        return in_array($clock_permission, ['own', 'clinic', 'admin']);
    }
}

/**
 * Verificar si el usuario puede acceder al módulo Clock-in (cualquier nivel)
 * @return bool
 */
if (!function_exists('can_access_clockin')) {
    function can_access_clockin()
    {
        $ci = new Security_Controller(false);
        
        // Los administradores del sistema pueden acceder a todo
        if ($ci->login_user->is_admin) {
            return true;
        }
        
        $clock_permission = get_array_value($ci->login_user->permissions, 'clock_permission');
        log_message('debug', 'Clockin can_access_clockin - clock_permission: ' . $clock_permission);
        log_message('debug', 'Clockin can_access_clockin - permissions array: ' . json_encode($ci->login_user->permissions));
        return in_array($clock_permission, ['own', 'clinic', 'admin']);
    }
}
