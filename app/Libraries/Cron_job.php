...
class Cron_job {
    ...
    public function alertar_marcajes_incompletos()
    {
        helper('clinics_helper');
        $clockInController = new Clockin();
        $clockInModel = new Clock_in_model();
        $clinicModel = new Clinic_model();

        $clockInController->syncClockInRecords(10000001);

        $fecha = date('Y-m-d', strtotime('-1 day'));
        
        $clinicas = $clinicModel->get_all();
        $usuariosAlerta = [];

        foreach ($clinicas as $clinica) {
            $usuarios = get_clinic_users($clinica->id);

            foreach ($usuarios as $usuario) {
                // ✔ Verificar si el usuario tiene al menos un marcaje en cualquier fecha
                $historial = $clockInModel
                    ->get_all_where([
                        'user_id' => $usuario->id,
                        'clinic_id' => $clinica->id
                    ])
                    ->getRow(); // traerá un solo registro si existe

                if (!$historial) {
                    continue; // usuario sin historial → no se toma en cuenta
                }

                // ✔ Obtener marcajes del día anterior
                $registrosHoy = $clockInModel->get_all_where([
                    'user_id' => $usuario->id,
                    'clinic_id' => $clinica->id,
                    'date' => $fecha
                ])->getResult();

                $cantidad = count($registrosHoy);
                ...
                if ($cantidad === 0) {
                    ...
                    $usuariosAlerta[] = [
                        ...
                        'clinic' => $clinica->name,
                        'marcajes' => $cantidad,
                        'mensaje' => "⚠️ Marcajes incompletos ($cantidad)"
                    ];
                }
            }
        }

        $fecha_formateada = date(get_setting('date_format'), strtotime($fecha)); 
        if (!empty($usuariosAlerta)) {
            /*$mensaje = "<h3>📋 Alerta de asistencia del día $fecha_formateada</h3><ul>";

            foreach ($usuariosAlerta as $u) {
                $mensaje .= "<li><b>{$u['nombre']}</b> ({$u['email']}) – Clínica: <b>{$u['clinic']}</b> – {$u['mensaje']}</li>";
            }

            $mensaje .= "</ul>";

            send_app_mail(
                'sebastyampi@gmail.com',
                "📢 Alerta de asistencia – $fecha_formateada",
                $mensaje
            );
           
            */

             $mensaje = "<h3>📋 📢 Alerta de Asistencia Clock In ($fecha_formateada)</h3><ul> </br>
            Nuestro sistema Clock In ha detectado un faltante de marcaje de los siguientes miembros:
            ";
            ...
            $mensaje .= "</ul> <br /> Gracias.";

            send_app_mail(
                'sebastyampi@gmail.com',
                "📢 Alerta de Asistencia Clock In ($fecha_formateada)",
                $mensaje
            );

             send_app_mail(
                'itsupport@rubymed.org',
                "📢 Alerta de asistencia – $fecha_formateada",
                $mensaje
            );
            send_app_mail(
            ...
            send_app_mail(
                'rlabanino@rubymed.org',
                "📢 Alerta de asistencia – $fecha_formateada",
                $mensaje
            );
            send_app_mail(
                'ultrasound@rubymed.org',
                "📢 Alerta de asistencia – $fecha_formateada",
                $mensaje
            );
        }
    ...
    private function call_daily_jobs() {
        ...
        if ($this->_is_daily_job_runnable()) {
            ...
            try {
                $this->alertar_marcajes_incompletos();
            } catch (\Exception $e) {
                echo $e;
            }
